...
function downloader(binDir, callback) {
  ...
  createBase(binDir);
  var detailsPath = path.join((binDir ? binBin : defaultBin), 'details');
  ...
  request.get(url, function get(err, res, body) {
    ...
    var details = fs.existsSync(detailsPath) && fs.readFileSync(detailsPath, 'utf8');
    ...
    download(link, function error(err) {
      ...
      fs.writeFileSync(detailsPath, JSON.stringify({version: newVersion, path: ((binDir) ? filePath : binDir), exec: exec('youtube-dl')}), 'utf8');
