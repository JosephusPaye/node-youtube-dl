 ...
 * @return {String}
 */
function getYtdlBinaryPath(binDir) {
  var defaultBin = path.join(__dirname, '..', 'bin');
  var detailsPath = path.join(binDir ? binDir : defaultBin, 'details');
  var ytdlBinary;

  if (fs.existsSync(detailsPath)) {
    var details = JSON.parse(fs.readFileSync(detailsPath));
    ytdlBinary = (details.path) ? details.path : path.resolve(binDir ? binDir : defaultBin, details.exec);
  }
...
function call(urls, args1, args2, options, callback) {
  ...
  var ytdlBinary = getYtdlBinaryPath(options.binDir);
...
ytdl.getSubs = function getSubs(url, options, callback) {
  ...
  call(url, args, [], { cwd: options.cwd, binDir: options.binDir }, function(err, data) {
    if (err) { return callback(err); }
